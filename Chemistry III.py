#Author Cowen M. Hames | April - May 2021. Version 3: February 2022.
#Periodic Table of the Elements Project: Elements, Facts, Symbols and Atomic Numbers
#Radioactivity graph removed and program structured to strictly OOP
from tkinter import *
...
class ChemistryGUI():
    ...
    def main():
        ChemistryGUI.window.mainloop()

    def get_Chemical_Properties():
        selection = ChemistryGUI.variable.get()
        for key, value in Chemistry.element_Symbol.items():
            if selection == key:
                ChemistryGUI.text_symbol.set(value)

        for key, value in Chemistry.element_Weight.items():
            if selection == key:
                ChemistryGUI.text_weight.set(value)

        for key, value in Chemistry.element_Group.items():
            if selection == key:
                ChemistryGUI.text_group.set(value)

        for key, value in Chemistry.element_Period.items():
            if selection == key:
                ChemistryGUI.text_period.set(value)

        for key, value in Chemistry.element_Atomic_Number.items():
            if selection == key:
                ChemistryGUI.text_AN.set(value)
        
        for key, value in Chemistry.element_Type.items():
            if selection == key:
                ChemistryGUI.text_type.set(value)

    def clear():
        ChemistryGUI.text_AN.set("")
        ChemistryGUI.text_period.set("")
        ChemistryGUI.text_group.set("")
        ChemistryGUI.text_weight.set("")
        ChemistryGUI.text_symbol.set("")
        ChemistryGUI.text_type.set("")
    ...
    variable.set(Chemistry.elements[0])
    ...
    om1 = OptionMenu(window, variable, *Chemistry.elements)
